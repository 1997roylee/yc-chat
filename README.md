...
A Next.js app that syncs Hacker News data and lets you chat with an AI assistant about what's trending. Two tabs: **AI Chat** (with persistent chatrooms) and **Story Feed** (browsable stories with comments).
...
The SQLite database is created automatically at `data/hn.db` on first run. To generate/push the schema manually:
...
1. Check if HN data is stale (>1 hour old) or empty
2. Automatically sync the top 50 stories + comments from Hacker News
3. Show an orange "Syncing HN data..." banner while syncing
